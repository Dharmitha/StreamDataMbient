...
data = genfromtxt('D:\Binary_normalized.csv', delimiter=',')
...
x1 = [data[index] for index in range(7600, valid_len)]
x1 = numpy.delete(x1, (data.shape[1]-1), axis=1)
x1 = numpy.array(x1)
y1 = [data[index][-1] for index in range(7600, valid_len)]
y1 = numpy.array(y1)
# reshape input and output data to be suitable for LSTMs
X1 = x1.reshape(1, valid_len-7600, data.shape[1]-1)
y1 = y1.reshape(1, valid_len-7600, 1)
